...
export class SecretsManager {
  private parseArn(arn: string): ParsedArn {
  ...
  async getSecrets(secretArn: string): Promise<SecretEntry[]> {
    try {
      ...
      const secrets = JSON.parse(response.SecretString) as SecretEntry[];
      return secrets;
    } catch (error) {
